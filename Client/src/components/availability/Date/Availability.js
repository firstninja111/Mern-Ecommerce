...
const Availability = (props) => {
  ...
  const [disabledDay, changeDisabledDay] = useState([]);
  ...
  useEffect(() => {
    ...
    var weekday = 1;
    filteredDataForCurrentStore.forEach(item => {
      if(Number(item.weekday) == weekday && Number(item.availability) > 0) {
        disabled.splice(disabled.indexOf(weekday), 1);
        weekday++;
      }
    })
    changeDisabledDay(disabled.map(item => item % 7));
  }, [getAllStoreAvailabilitiesData])
  ...
  return (
    <div className="availability_container" id="date_page">
      ...
      <Calendar
        ...
        tileDisabled={({ date }) => disabledDay.includes(Number(date.getDay())) }
