...
const AdminEditStaffMember = (props) => {
  ...
  const [storeList, changeStoreList] = useState(props.item.store.name ? [props.item.store.name ] : []);
        ...
        ((styleprops) => (
          ...
          >
            ...
            <div className="admin_individual_selected_employee_contents">
              ...
              {roleList.length > 0
                ? roleList.map((role, index) => (
                    ...
                    >
                      ...
                      {getEmployeeData.employee.employeeRole.includes("Admin") || getEmployeeData.employee.employeeRole.includes("Manager")
                        ? 
                          <FontAwesomeIcon
                            icon={faTimes}
              ...
              {roleList.length < 3  && (getEmployeeData.employee.employeeRole.includes("Admin") || getEmployeeData.employee.employeeRole.includes("Manager"))
                ? (
                <div className="admin_create_appointment_input_information_container">
                  ...
                  <Dropdown
                    options={getAllRolesData.all_roles.map(item => item.name)
                      .filter((x) => {
                        ...
                        else return !roleList.includes(x) 
                      })}
                    onChange={(choice) => {
                      resetAllErrorStates();
