...
import "./AdminInProgressNotifications.css";

const AdminInProgressNotifications = (props) => {
  const {
    getEmployeeData,
    getEmployeeLoading,
    getEmployeeError,
  ...
  const renderNoNotifications = () => {
    return (
      <div className="my_upcoming_appointments_empty_container">
        ...
        <p>You have no in-progress appointment to show.</p>
      </div>
    );
  };
  ...
  return (
    <div className="admin_notifications_container">
      ...
      >
        ...
        {!addWaitingUserClicked && !clickedNotification && <h1>In Progres</h1>}
        ...
        </div>
        {
          !getEmployeeData ? (
            ...
            getAllAppointmentsData.all_appointments.filter((e) => {
              if((!getEmployeeData || !getEmployeeData.employee || !getEmployeeData.employee.employeeRole || getEmployeeData.employee.employeeRole.includes("Admin")) && e.status == "in-progress") { 
                return true;
              }
              if(e.store._id === getEmployeeData.employee.store._id && e.status == "in-progress") {
                return true;
              }
              return false;
            }).length > 0 ?
...
export default AdminInProgressNotifications;
