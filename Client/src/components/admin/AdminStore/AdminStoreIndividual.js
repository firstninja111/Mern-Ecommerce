...
const AdminStoreIndividual = (props) => {
  ...
  const [deletable, changeDeletable] = useState(false);
  ...
  return (
    <>
      ...
      >
        <div className="admin_individual_selected_store_info_container">
          <h1>
            {props.item.name}
          </h1>
        </div>
        <div className="admin_individual_selected_store_info_container">
          <p>Address: {props.item.address}, {props.item.city}, {props.item.country}</p>
          <p>Email: {props.item.email}</p>
          <p>Phone: {props.item.phone}</p>
          <p>Website: {props.item.website}</p>
        </div>
      </div>
        ...
        {props.getEmployeeData ? 
          props.getEmployeeData.employee
            ? (
                ...
                props.getEmployeeData.employee.store.name == props.item.name
              )
            ? 
              <div
      ...
      <div className="admin_client_profile_bottom_buttons_container">
        {props.item.manager == "No manager!" && props.getEmployeeData ? 
          props.getEmployeeData.employee
            ? (
                props.getEmployeeData.employee.employeeRole.includes("Admin")
              )
