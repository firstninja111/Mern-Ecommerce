...
import getAllStoreAvailabilitiesQuery from "./graphql/queries/getAllStoreAvailabilitiesQuery";
...
import ACTION_TEMPORARY_DUMMY_TOKEN from "./actions/Login/TemporaryDummyToken/ACTION_TEMPORARY_DUMMY_TOKEN";
import ACTION_TEMPORARY_DUMMY_TOKEN_RESET from "./actions/Login/TemporaryDummyToken/ACTION_TEMPORARY_DUMMY_TOKEN_RESET";
...
const App = () => {
  ...
  const temporaryDummyToken = useSelector(
    (state) => state.temporaryDummyToken.temporary_dummy_token
  );
  ...
  const {
    data: getAllStoreAvailabilitiesData,
    refetch: getAllStoreAvailabilitiesRefetch,
    loading: getAllStoreAvailabilitiesLoading,
    error: getAllStoreAvailabilitiesError,
  } = useQuery(getAllStoreAvailabilitiesQuery, {
    fetchPolicy: "no-cache",
  });
  ...
  useEffect(() => {
    ...
    const checkCookies = () => {
      ...
      if (
        ...
        temporaryClientDummyToken !==
          Cookies.get("temporary-dummy-token")
      ) {
        currentDummyToken = Cookies.get("dummy-token");
        ...
        temporaryClientDummyToken = Cookies.get("temporary-dummy-token");
        ...
        if (currentDummyToken) {
          ...
          } else if (currentDummyToken) {
            if (!dummyToken) {
              dispatch(
                ACTION_DUMMY_TOKEN(jwt.decode(currentDummyToken))
              );
            }
          ...
          } else if (temporaryClientDummyToken) {
            if (!temporaryDummyToken) {
              dispatch(
                ACTION_TEMPORARY_DUMMY_TOKEN(
                  jwt.decode(temporaryClientDummyToken)
                )
              );
            }
            if (adminAuthenticated) {
  ...
  const shoppingCartConditionalActiveRendering = () => {
    ...
    if (cartPageOpened === "Availability") {
      return (
        <Suspense fallback={renderCartRoutesFallbackLoader()}>
          <Availability
            ...
            initialScreenSize={initialScreenSize}
            getEmployeesData={getEmployeesData}
            getAllStoreAvailabilitiesData={getAllStoreAvailabilitiesData}
            getAllStoreAvailabilitiesRefetch={getAllStoreAvailabilitiesRefetch}
            getAllStoreAvailabilitiesLoading={getAllStoreAvailabilitiesLoading}
            getAllStoreAvailabilitiesError={getAllStoreAvailabilitiesError}
          />
        </Suspense>
      );
    } else if (cartPageOpened === "TimePreference") {
