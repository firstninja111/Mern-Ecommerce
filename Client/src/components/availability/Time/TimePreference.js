...
const TimePreference = (props) => {
  const {
    selectedStore,
    getAllStoreAvailabilitiesData,
    getAllStoreAvailabilitiesRefetch,
    getAllStoreAvailabilitiesLoading,
    getAllStoreAvailabilitiesError,
  } = props;
  ...
  useEffect(() => {
    if (getAllStoreAvailabilitiesLoading) {
      dispatch(ACTION_LOADING_SPINNER_ACTIVE());
    } else {
      dispatch(ACTION_LOADING_SPINNER_RESET());
    }
  }, [dispatch, getAllStoreAvailabilitiesLoading]);

  useEffect(() => {
    if (getAllStoreAvailabilitiesError) {
      getAllStoreAvailabilitiesRefetch();
    }
  }, [getAllStoreAvailabilitiesError, getAllStoreAvailabilitiesRefetch]);
  ...
  const alreadyBookedAppointments = appointmentsData
    ? appointmentsData.all_appointments.filter(
        (item) =>
          ...
          item.store.name === selectedStore.name
  ...
  const alreadyBookedTimes = useCallback(() => {
    ...
    if (alreadyBookedAppointments !== null) {
      ...
      let allAlreadyBookedTimesList = alreadyBookedTimesArr.reduce(
        (accumulator, currentValue) => accumulator.concat(currentValue),
        []
      )
      var uniqueBookedList = [...new Set(allAlreadyBookedTimesList)];
      
      // duplication counts
      const duplicationCounts = {};
      allAlreadyBookedTimesList.forEach((x) => { duplicationCounts[x] = (duplicationCounts[x] || 0) + 1; });
      // compare availability with the store
      var filteredDataForCurrentStore = getAllStoreAvailabilitiesData.all_storeAvailabilities.filter(item => {
        return item.location == selectedStore.name
      });
      // disable time for schecule
      var disabledTimes = [];
      filteredDataForCurrentStore.forEach(availability => {
        if(Number(availability.weekday) % 7 === selectedDay.getDay() && Number(availability.availability) == 0){
          disabledTimes.push(availability.hour + ":00");
          disabledTimes.push(availability.hour + ":15");
          disabledTimes.push(availability.hour + ":30");
          disabledTimes.push(availability.hour + ":45");
        }
      })
      ...
      uniqueBookedList.forEach(time => {
        var hour = time.split(":")[0];
        var availabilityAtThisTime = filteredDataForCurrentStore.filter(item => {
          return Number(item.weekday) % 7 === selectedDay.getDay() && Number(item.hour) === Number(hour)
        })[0]?.availability;

        if(duplicationCounts[time] < Number(availabilityAtThisTime)) disabledTimes.splice(disabledTimes.indexOf(time), 1);
      })
      changeBookedTimes([...new Set(disabledTimes)]);
    }
  ...
  const createMorningTimeInstances = () => {
    ...
    for (let i = 0; i < multipleHours.length; i++) {
      morningTimesArr.push(
        multipleHours[i].toString() + ":" + minutes[i % 4].toString()
      );
    }
  };
  ...
  return (
    <div className="select_time_container" id="time_page">
      ...
      <div className="time_of_day_selectors_wrapper">
        ...
        {dayOfTheWeek === "Friday_disabled" ? null : (
          <div className="time_of_day_card_container">
            <div className="time_of_day_card_top_wrapping">
              <div className="time_of_day_heading">
                <p>Evening</p>
