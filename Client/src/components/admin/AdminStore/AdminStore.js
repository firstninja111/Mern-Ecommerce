...
const AdminStore = (props) => {
  ...
  useEffect(() => {
    if(getAllServicesData){
      let temp = {};
      getAllServicesData.all_services.forEach(e => {
        temp[e._id] = e.name;
      });
      changeServices(temp);
    }
    ...
    if (getAllStoresData) {
      if (getAllStoresData.all_stores.length > 0) {
        if (changeStoreFilter) {
            let filtered = [...getAllStoresData.all_stores].filter((x) => {
              return (
          ...
          changeFilteredAllStores(
            [...filtered].map((store) => {
              let manager = getEmployeesData.employees.filter((employee) =>  {
                return employee.store._id == store._id && 
                  (employee.employeeRole.includes("Admin") || employee.employeeRole.includes("Manager"));
              });
              return {
                ...store,
                manager: manager.length ? manager[0].firstName +" " + manager[0].lastName : "No manager!",
              }
            })
          )
        }
    ...
    <div className="admin_clients_container">
      ...
      >
        <div className="admin_individual_client_container admin_list_header">
          <div className="admin_individual_client_full_name admin_store_information">
            <div>
              <b>Name</b>
            </div>
            <div>
            ...
            <div>
              <p style={{width: "fit-content"}}>Contact Info</p>
            </div>
            <div>
              <p style={{width: "fit-content"}}>Website</p>
            </div>
            <div>
              <p style={{width: "fit-content"}}>Services</p>
            </div>
            <div>
              <p style={{width: "fit-content"}}>Manager</p>
            </div>
          </div>
        </div>
        {getAllStoresData && getEmployeeData
          ? getAllStoresData.all_stores.length > 0
            ? filteredAllStores
                .sort((a, b) => a.name.localeCompare(b.name))
                .map((item, i) => {
                  if(getEmployeeData.employee.employeeRole.includes("Admin") || item._id == getEmployeeData.employee.store._id){
                    return <div
                    ...
                    >
                      <div className="admin_individual_client_full_name admin_store_information">
                        ...
                        <div>
                          <p style={{width: "fit-content"}}>{item.website}</p>
                        </div>
                        <div>
                          {item.availableServices.length > 0
                            ? item.availableServices.map((service, i) => <p key={i} style={{width: "fit-content", color: "rgb(0, 0, 0)"}}>{services[service]} </p>) 
                            : null
                          }
                        </div>
                        <div>
                          <p style={{width: "fit-content"}}>{item.manager}</p>
                        </div>
                      </div>
                      {adminSelected === "" ? (
                        <Transition
