...
cron.schedule("* * * * *", async () => {
  ...
  if (dayPriorMatchArr.length > 0) {
    dayPriorMatchArr.forEach((appointment) => {
      ...
      //       "! This is a reminder for your CHOIREDEX appointment tomorrow, " +
    ...
    hourPriorMatchArr.forEach((appointment) => {
      ...
      //       appointment.client.firstName[0].toUpperCase() +
      //       appointment.client.firstName.slice(1).toLowerCase() +
      //       "! We look forward to seeing you at your CHOIREDEX appointment today at " +
      //       appointment.startTime +
      //       ". " +
...
var connectUrl = `mongodb://${process.env.MONGO_DB_USERNAME}:${process.env.MONGO_DB_PASSWORD}@${process.env.MONGO_DB_HOSTNAME}/glowlabsdb?authSource=admin`;
if(process.env.NODE_ENV === "production") connectUrl = `mongodb://${process.env.MONGO_DB_USERNAME}:${process.env.MONGO_DB_PASSWORD}@${process.env.MONGO_DB_HOSTNAME}:27017/glowlabsdb?authMechanism=DEFAULT&authSource=admin&retryWrites=true&w=majority`;
...
app.use(async (req, res, next) => {
  ...
  const temporaryAccessToken = req.cookies["temporary-access-token"];
  const temporaryDummyToken = req.cookies["temporary-dummy-token"];
  const logoutCookie = req.cookies.logout;
  ...
  if (!accessToken && !refreshToken && !temporaryFacebookAccessToken && !temporaryAccessToken) {
    ...
    if (!refreshToken) {
      ...
      if (temporaryFacebookAccessToken) {
        ...
        req.isAuth = true;
        const client = await Client.findOne({
          _id: jwt.decode(temporaryAccessToken).id,
        });

        if (client.permanentPasswordSet) {
          const tokens = createTokens(client);
          res.clearCookie("temporary-access-token", {
            domain:
              process.env.NODE_ENV === "production"
                ? process.env.PRODUCTION_CLIENT_ROOT
                : "localhost",
          });
          res.clearCookie("temporary-dummy-token", {
            domain:
              process.env.NODE_ENV === "production"
                ? process.env.PRODUCTION_CLIENT_ROOT
                : "localhost",
