...
const AdminServiceIndividual = (props) => {
  ...
  const [deletable, changeDeletable] = useState(true);
  ...
  useEffect(() => {
    if(props.getAllStoresData){
      if(props.getAllStoresData.all_stores){
        props.getAllStoresData.all_stores.forEach(store => {
          if(store.availableServices.includes(props.item.name)) changeDeletable(false)
        });
      }
    }
  }, [props])
  ...
  return (
    <>
      ...
      <div className="admin_client_profile_bottom_buttons_container">
        {deletable && props.getEmployeeData ? 
          props.getEmployeeData.employee ? 
            props.getEmployeeData.employee.employeeRole.includes("Admin") ? 
              <div
                className="profile_button_container"
