...
import "./StoreSelect.css";
 ...
 const StoreSelect = (props) => {
  ...
  const [serviceIds, changeServiceIds] = useState({});
        ...
        if (selectedServiceName) {
          changeFilteredStored(
            [...getAllStoresData.all_stores].filter((item) => {
              return item.availableServices.includes(serviceIds[selectedServiceName])
            })
          );
        }
      }
    }
  }, [selectedServiceName, getAllStoresData, serviceIds]);
  ...
  }, [dispatch, getAllStoresRefetch, getAllStoresData, getAllStoresLoading]);
